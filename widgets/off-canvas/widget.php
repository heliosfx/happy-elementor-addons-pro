...
class Off_Canvas extends Base {
	...
	protected function register_ha_content_close_bar_controls(){
		...
		$this->add_control(
			...
			[
				'label' => __( 'Editional Content', 'happy-addons-pro' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'none',
				'options' => [
	...
	protected function render_close_button() {
		...
		$link = $settings['close_bar_link'];
		$target = $settings['close_bar_link']['is_external'] ? ' target="_blank"' : '';
		$nofollow = $settings['close_bar_link']['nofollow'] ? ' rel="nofollow"' : '';
		$content_type = $settings['close_bar_additional'];

		$logo = $settings['close_bar_logo']['url'];
